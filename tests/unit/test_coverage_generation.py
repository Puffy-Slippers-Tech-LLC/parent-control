...
def test_shared_scenario_titles_identify_each_variant(document):
    ...
    for number, variant, description in (
        ...
        assert f'| [{number}](#scenario-{number}) | {title} | `E2E-003/{variant}` | ready |' in rendered
...
def test_generation_replaces_entire_document_and_is_deterministic(checkout, monkeypatch):
    monkeypatch.setattr(coverage, 'collect_counts', lambda root: [('Unit', 7, 'Collected cases')])
    ...
    assert b'| Unit | <span style="color: green">7</span>/<span style="color: gray">0</span>/7 |' in first
    assert b'| E2E runner-smoke | <span style="color: green">1</span>/<span style="color: gray">0</span>/1 |' in first
