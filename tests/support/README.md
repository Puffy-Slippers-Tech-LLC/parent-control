...
| Preview process lifecycle | [preview.py](preview.py) | `preview_applications` owns only its Popen handles/logs, cleans up failed discovery, attempts every owned child and reports cleanup failures. `tests/ui/conftest.py` owns the compositor, accessibility and `request_display_scale` fixtures; layout/overflow tests discover scaling there instead of importing a collected case. |
