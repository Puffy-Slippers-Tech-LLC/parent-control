...
sub run {
    onpc_progress::operation('Preparing a kiosk request and escaping to sign-in');
    my ($exchange) = @_;
    die 'kiosk-escape:arguments' unless @_ == 1 && ref($exchange) eq 'CODE';
    my $journey = onpc_journey->new(exchange => $exchange, prefix => 'kiosk-escape', review => 0);
