...
| A2 `password_recipient`; `fixture_credentials`; functional `onpc_password` recipient proofs | Parent/standard login and wrong-recipient qualification | GDM/Polkit app/surface/recipient/empty masked field IDs | **Contained (03).** The selected-recipient/password IDs own the sole focused, enabled, showing, empty masked-field proof. Wrong-recipient refusal, two fresh checks, secret sealing/single use and uncertainty latches passed unchanged. Legacy masked-image callers remain L6/L7. | Complete (03); provider blocked |
| A3 `system_prompt_control/absent/handle_system_prompt`, `wait`, `run` preamble | Every non-greeter operation with prompt middleware; retained coordinate rendezvous | gcr/keyring and Shell Polkit app/dialog/cancel/secret/recipient IDs | **Contained (03).** Both mappings preflight before traversal. Qualified keyring Cancel uses one semantic action and ID-scoped complete disappearance; Polkit presence and unknown/unqualified paths refuse. Worker coordinate emission is removed. The unreachable coordinate parser/rendezvous is recorded under L2/L3 for deletion, never revival. | Complete (03); provider blocked |
| D1 `desktop_result`, `session_menu_toggle/power/menu`, `choose_session_action`, `logout_confirm` | `onpc_desktop_session`, login/terminal workers | Shell desktop/panel/session menu/action IDs | **Contained (04).** Every adapter entry preflights the Shell application mapping and scopes qualified doubles through desktop, panel, session-menu or confirmation surface IDs. No adapter result exports geometry. Retained Perl pointer callers remain L2/L3/L10 and cannot become reachable while the installed provider mapping is absent. | Complete (04); provider blocked |
| D2 `launchable_result`, `search_query/ready/result/absence`, `wait_search`, `search_diagnostic`, search branches of `read_label`/`run` | Parent launch/access, `onpc_parent`, nested `e2e_search_probe` | Shell overview/search/result/web-suggestion IDs | **Contained (04).** Query, focus, launchability, web witness and stable launcher/window absence are ID-scoped; text/roles only verify identified nodes. Diagnostics report only identified logical controls. Missing application IDs refuse before traversal, including the retained nested probe. Direct Shell input remains L5/L9. | Complete (04); provider blocked |
| D3 `terminal_input/focus_terminal/help_terminal_text/help_shell_prompt/help_content/help_product_absent`, terminal/help `run` branches | `onpc_terminal`, `onpc_parent_terminal`, `onpc_command_help` | Terminal app/window/input-output IDs; product root IDs for exclusion | **Contained (04).** The registered terminal surface and input/output ID own active/focused input, bounded output and complete return/absence checks. Wrong-surface, duplicate, inactive and missing-provider cases refuse; product exclusion remains by product IDs. Retained shortcut/typing keys remain L10. | Complete (04); provider blocked |
| D4 `license_content/read_document/open_license`, license branches of window close/return | `onpc_parent_about`, ABOUT02/03 | Registered viewer app/window/document/close IDs | **Contained (04).** The complete viewer mapping is required before the owned license link can launch its destination. Content, active-window readiness and complete return use viewer surface/document/close IDs plus the owned About destination; title/frame discovery is removed. Retained Alt-F4 remains L10. | Complete (04); provider blocked |
...
| L2 `pointer_target/pointer_glyph/stable_pointer`, `UiObservations` pointer validation, installed-journey `ui_pointer` dispatch | Session/search workers; unreachable former prompt rendezvous | Semantic action on identified control, no coordinates | Extents/minimum-box/settling and coordinate payloads remain callable. Task 03 removed prompt production/consumption reliance; task 05 must delete that unreachable prompt route rather than reconnect it. | 05 |
| L3 `onpc_journey.observe/seen/navigate_choice/service_system_prompt/click_target` | All graphical worker families, review branch too | ID observation/action and independent focus proof | Image gates, positional navigation and pointer input remain reachable. `service_system_prompt` is unreachable after task 03 and is deletion-only task-05 scope; preserve generic rendezvous/no-replay obligations for migrated consumers. | 05 |
...
desktop/external 0, legacy/transport 10 (L1–L10), fixture 2 (F1–F2)**.
D1–D6 are contained on recorded provider blockers, with live consumers pending;
the legacy rows are not yet safely contained. R1–R4 record
