"""Execute the actual test-runner Polkit rule against allowed and excluded requests."""
...
RULE = Path(__file__).resolve().parents[2] / 'config/50-onpc-test-runner.rules'


@pytest.mark.parametrize('override,allowed', [
    ({}, True),
    ({'program': '/usr/bin/make'}, False),
    ({'program': '/usr/bin/python3'}, False),
    ({'program': '/usr/local/libexec/onpc-export-screenshot'}, False),
    ({'program': '/tmp/onpc-test-runner'}, False),
    ({'program': '/usr/local/libexec/onpc-test-runner-other'}, False),
