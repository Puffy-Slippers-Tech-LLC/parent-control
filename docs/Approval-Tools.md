...
| Scripted E2E implementation | `tools/write-e2e [--sessions N] [--tasks N]` / `tools/write-e2e --stop` | Fresh Astra Low implementation and Astra High live/repair sessions; unlimited sessions when omitted, completed tasks default to 1; stop at either limit; attach to a live owner before interpreting new arguments; safe stop at the next session boundary, Ctrl+C owned cancellation; task-scoped staging without commits; existing grants and guarded test cleanup |
