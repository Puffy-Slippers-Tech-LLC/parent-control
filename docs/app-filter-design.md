...
After an approved unchecked request, the broker first verifies that complete blocklist, then terminates matching native, Snap, and Flatpak applications owned only by the approved child's UID, and writes `ActiveExtension` last. An approved checked request performs no process termination at all. Native and Snap PID signalling uses pidfds after verifying all reported process UIDs; Snap matching additionally uses the kernel-applied application security label. Flatpak enumeration and instance-specific termination run with the approved child's identity and runtime directory, so an identical application owned by another account is out of scope.
