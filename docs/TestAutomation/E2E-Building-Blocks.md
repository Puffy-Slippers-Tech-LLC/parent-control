...
| PARENT05 | C | Open the daily-allowance picker and, when requested, its Custom amount editor. | `AccessibleUI.allowance_preset(child, minutes, action=...)` implements fixed 0/15 select, independent saved read and reopened-picker selected-description readback through public IDs and PARENT08. Selection waits for picker closure before reading the selector; `reopen` verifies the saved selector first and returns the picker open. The public `menu.popup` action is not a close toggle. `allowance_presets.PLAN` / `onpc_allowance_presets::run` / `check_e2e_allowance_presets` qualified fresh entry, both presets, independent saved and reopened-selected readback, wrong-child and disabled-control refusal in run `20260925T044650Z-53e3ca11`, with collection, owned cleanup and baseline restoration. Custom input belongs to task 040; other values and complete scenarios remain pending. | pending; preset 0/15 slice qualified |
