...
| Scripted test repair | `tools/fix-tests [CATEGORY ...] [--model MODEL] [--effort high]` / `tools/fix-tests --stop` | Granular pass then complete regression retries by default; explicit categories expand to leaves and restrict repair and verification to that scope; detached scripting owner, one fresh ephemeral agent per repair, existing sandbox/rules and test-runner cleanup; no automatic setup or authority expansion |
