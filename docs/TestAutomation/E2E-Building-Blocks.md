...
| PARENT05 | C | Open the daily-allowance picker and, when requested, its Custom amount editor. | `AccessibleUI.allowance_preset(child, minutes, action=...)` implements fixed 0/15 select, independent saved read and reopened-picker selected-description readback through public IDs and PARENT08. Selection waits for picker closure before reading the selector; `reopen` verifies the saved selector first and returns the picker open. The public `menu.popup` action is not a close toggle. `allowance_presets.PLAN` / `onpc_allowance_presets::run` / `check_e2e_allowance_presets` qualified that preset slice in run `20260925T044650Z-53e3ca11`. `AccessibleUI.custom_allowance(child, minutes, action=...)` opens and reopens the custom editor. `check_e2e_allowance` qualified custom 1/2/3 and repeated both presets in run `20260925T053812Z-45e24883`, including fresh entry, saved and child-reloaded readback, wrong-child and disabled-control refusal, collection, owned cleanup and baseline restoration. Other values and complete scenarios remain pending. | pending; preset 0/15 and custom 1/2/3 slices qualified |
| PARENT06 | C | Choose a daily preset or type a custom allowance and commit through the normal UI. Return the displayed value/validation; saving is observed separately. | Preset: PARENT05(picker) → UI04(preset) → UI03. `allowance.PLAN` / `onpc_allowance::run` binds custom 1/pause, 2/Enter and 3/Tab using `onpc_text::replace_text` and UI16 `daily-1/2/3` bindings. A single focused, bounded keyboard batch includes the Enter/Tab terminator before debounce can disable the editor; pause adds no input. `AccessibleUI.custom_allowance` separately observes PARENT08, then child reselection reloads saved values before reopened-editor readback. Saving may clear focus; focus retention is not a save result. `check_e2e_allowance` qualified all three routes in run `20260925T053812Z-45e24883`, including independent exact saved/reopened readback, refusal checks, collection, owned cleanup and baseline restoration. Maximum/invalid values and complete scenarios remain pending. | pending; ordinary custom commit routes qualified |
