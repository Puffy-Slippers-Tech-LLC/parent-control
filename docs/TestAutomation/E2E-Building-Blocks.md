...
| ID | Kind | Block and explicit contract | Callees / reuse source | Status |
| --- | --- | --- | --- | --- |
| GDM08 | A | Observe the selected account label, focused showing password role and hidden account list. This is a nonsecret prompt observation only; it cannot authorize password input and never reads password contents. | `AccessibleUI.greeter_prompt()` uses the scoped semantic GDM adapter to resolve the sole greeter, exact prepared recipient and protected password node while rejecting visible account rows. `tools/run-tests integration check_e2e_gdm_recipient` installed-qualifies the Parent prompt; focused tests reject wrong/duplicate recipients, missing/duplicate/unfocused/nonempty/unmasked fields and list overlap without traversing or reading password text. | pending; prepared Parent prompt qualified |
...
| GDM01 | C | Observe the usable greeter and its account list, with no active password prompt. | `AccessibleUI.gdm_nonsecret_account(name)` resolves the sole active greeter owner, exact prepared ordinary row and distinct station row from one complete list while excluding an overlapping password prompt. `check_e2e_gdm_navigation` qualifies Parent list/return cycles; `check_e2e_gdm_recipient` additionally qualifies the prepared other-parent list and fresh Parent/other-parent returns. Wrong-owner, duplicate-account, stale and overlap paths refuse. Other ordinary accounts and surfaces remain unqualified. | pending; prepared Parent/other-parent list bindings qualified |
| GDM02 | C | Select a named user on the logon screen. Use UI14's qualified provider navigation, verify focus before Enter, then observe the declared password prompt, retained lock or passwordless station. Do not type a password. | `AccessibleUI.gdm_nonsecret_navigation(name)` semantically focuses and freshly verifies the exact prepared row before `onpc_gdm` sends one Enter. `check_e2e_gdm_navigation` qualifies two Parent cycles; `check_e2e_gdm_recipient` qualifies other-parent and Parent prompt entry; `check_e2e_kiosk_entry` separately qualifies the station row and passwordless owned-form result. Other accounts, retained locks and session choices remain unqualified; positional Home/Down calculation stays retired. | pending; prepared Parent/other-parent and passwordless station bindings qualified |
| GDM09 | C | Dismiss an already observed GDM password prompt with one Escape and independently observe the account list again. No secret is typed. | `onpc_gdm::recipient_qualification` consumes each scoped prompt proof before one Escape and obtains a fresh complete returned-list proof. The installed qualification covers the prepared other-parent and Parent prompts; retained locks and other prompt surfaces remain unqualified. | pending; prepared Parent/other-parent bindings qualified |
...
| GDM greeter | The prepared Ubuntu 26.04 image's English semantics and baseline keyboard configuration are qualified for unique Parent/other-parent/station list ownership, fresh semantic focus, prepared ordinary prompt entry, exact empty masked Parent/other-parent recipients, wrong-recipient refusal and Escape/list return. `check_e2e_gdm_recipient` supplies the installed GDM01/02/03/04/08/09 proof and owned cleanup ([report](Evidence/test-all-runs/20260922T151443Z-6d456ce8/report.md)); `check_e2e_kiosk_entry` separately qualifies one passwordless station Enter and owned-form readback (GNOME Shell `50.1-0ubuntu1.2`). Password submission, standard-child entry, retained locks, session choices and other GDM surfaces remain unqualified. | REQUEST01 and the prepared GDM01–04/08/09 bindings are ready; GDM05–07 and other GDM routes remain pending |
