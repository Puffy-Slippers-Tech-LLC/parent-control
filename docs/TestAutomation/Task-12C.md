...
- Complexity: medium-high. The code is complete, but the operator intentionally
  performs the two privileged preparation commands against the real VM and host.
- Recommended Codex model: `gpt-5.6-terra`
- Recommended reasoning effort: `high`
- Execution locations: `make prep-vm` is run manually by the developer inside
  the existing `ubuntu26.04` guest checkout. `make prep-host` is then run
  manually by the developer in the development-host checkout. Codex performs
  only read-only inspection and host-safe verification after the developer says
...
- Operator sequence:
  1. Confirm that the terminal is inside the intended `ubuntu26.04` VM, enter a
     root shell, change to the already-copied repository, run `make prep-vm`, and
     enter the shared password once at its no-echo prompt. Do not install the
     product.
  2. Return to the development/libvirt host, change to its repository checkout,
     ...
     or resume the baseline capture. Do not run `make installdeb` on the host.
  3. Report completion and the baseline/provenance paths to Codex. If either
     ...
     do not delete partial files or alter libvirt manually.
- Codex verification after operator completion:
  1. Read the preparation record, capture phase record, provenance document,
     ...
     read-only commands. Confirm the source domain is shut off and its disk chain
     was not changed by capture.
  2. Confirm the baseline is an independent, non-writable QCOW2 image, passes
     ...
     installed Oh No! Parent Control package or payload.
