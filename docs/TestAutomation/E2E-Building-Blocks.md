...
| GDM01 | C | Observe the usable greeter and its account list, with no active password prompt. | `AccessibleUI.gdm_nonsecret_account(parent)` uses the scoped GDM semantic adapter to resolve the sole active greeter owner and one exact ordinary row in a complete list, while requiring the distinct station row and excluding an overlapping password prompt. `tools/run-tests integration check_e2e_gdm_navigation` installed-qualifies the ordinary Parent list initially and after Escape, then repeats from an independently supplied valid list; wrong-owner, duplicate-account and list/prompt-overlap paths refuse. Other ordinary accounts and surfaces remain unqualified. | pending; ordinary Parent list binding qualified |
| GDM02 | C | Select a named user on the logon screen. Use UI14's qualified provider navigation, verify focus before Enter, then observe the declared password prompt, retained lock or passwordless station. Do not type a password. | `AccessibleUI.gdm_nonsecret_navigation(parent)` through `onpc_gdm::navigation_qualification` is installed-qualified by `tools/run-tests integration check_e2e_gdm_navigation`: each of two independent cycles freshly resolves the exact ordinary row, focuses it semantically, verifies focus, presses Enter once, observes a fresh nonsecret Parent prompt, presses Escape once and independently reads the returned list. `check_e2e_kiosk_entry` separately qualifies the station row and passwordless owned-form result. Recipient/secret proofs, other accounts, retained locks and session choices remain unqualified; the retained generic positional Home/Down calculation stays retired. | pending; ordinary Parent and passwordless station bindings qualified |
...
| GDM greeter | The prepared Ubuntu 26.04 image's English station semantics and baseline keyboard configuration are qualified for unique ordinary/station list ownership, wrong ordinary-entry refusal, fresh station focus, one passwordless Enter and owned-form readback by `check_e2e_kiosk_entry` (GNOME Shell `50.1-0ubuntu1.2`). Other-account entry, secret recipients, prompt dismissal and session choices remain unqualified. | REQUEST01 ready for the passwordless station branch; GDM01–09, fresh login and other GDM routes remain pending |
