...
Ordinary child overlay entry uses REQUEST02 to invoke the installed
`oh-no-parent-control-child` command directly as the active child desktop user.
No terminal or search is involved. FILE01 remains available for cases that
actually need Terminal; command-help reads guarded SSH stdout.
...
| SEC01 | A | Fresh scoped semantic recipient proofs remain mandatory. Preserve wrong-recipient, focus, empty-field and capture protections; images and elapsed time cannot supply proof. | `AccessibleUI.password_recipient` is installed-qualified for the prepared Parent/other-parent GDM binding without reading password contents. Secret delivery and the remaining customer GDM07 route still require their own qualification; legacy `onpc_parent::login` and `onpc_password::enter_password` continue to refuse before image/backend/secret access. | pending; prepared GDM recipient proof qualified |
| SEC02 | A | Account selection requires qualified provider-specific selection and independent password-recipient qualification. | `AccessibleUI.gdm_nonsecret_navigation` and `password_recipient` are installed-qualified for the prepared Parent/other-parent binding. Legacy Parent/GDM selectors and generic `onpc_pointer::click` still refuse before image/backend/input access; no generic fixed-coordinate fallback or new needle is allowed. | pending; prepared GDM selection/recipient qualified |
...
| GDM04 | C | Observe a different account's empty GDM prompt, prove it is refused as the intended secret recipient, dismiss it, and observe the account list again. The declared wrong account has no retained desktop, so selection reaches a GDM prompt. | `onpc_gdm::recipient_qualification` selects the prepared other-parent, proves its real empty prompt, refuses it as Parent, consumes that proof before one Escape and independently observes the returned list. Wrong-owner, duplicate-account, stale/replayed-proof and field-state checks refuse. | ready; prepared other-parent-to-Parent binding qualified |
| GDM05 | C | Type the intended user's password into the already selected GDM prompt. Require the explicit wrong-recipient evidence and two fresh ordered recipient checks immediately before one secret input. Do not submit. | `onpc_password::enter_parent_gdm_password` / `enter_standard_gdm_password` retain sealed single-use input after the two `AccessibleUI.password_recipient` proofs. `FreshDesktopJourney` and `onpc_fresh_desktop::run` use separate prepared Parent and standard entries; `check_e2e_fresh_desktop` qualified both wrong-recipient refusals, ordered secret delivery and later independent desktop results with owned cleanup. `check_e2e_desktop_keyring` requalified both entries and added the standard desktop's prepared real-gcr Cancel branch through `KeyringDesktopJourney`. Other secret surfaces and retained-session entry remain pending. | pending; fresh Parent/standard entry ready |
...
| REQUEST01 | C | Enter the dedicated request station from GDM and observe its request form. Use only the documented station/session selection, without a desktop-login shortcut. | `check_e2e_kiosk_entry` uses `AccessibleUI.gdm_nonsecret_navigation` and consumes one fresh focused-station proof before Enter; `station_default_entry` independently requires the active station owner and one showing owned window/form. The installed passwordless default branch is qualified for the prepared Ubuntu 26.04/English-GDM/baseline-keyboard image with GNOME Shell `50.1-0ubuntu1.2`; other GDM accounts, secrets and session choices remain separate. | ready |
...
| REQUEST03 | C | Read a form's child, approver, duration, custom text, soft-app choice, controls and messages; observe the absence of mute in the current release. Require exactly one showing form and the fixed child in overlay. | UI13(form count=1) → UI01 → UI02 → UI03. `AccessibleUI.kiosk_request_form` is installed-qualified for the fixed disabled-child kiosk projection: two complete owned-ID snapshots return one immutable form, its disabled selectors/duration/soft-app/Request states, notice, Cancel state and absent mute/custom value. Enabled kiosk choices, list exclusions and overlay fixed-child scope remain pending. | pending; kiosk disabled-child projection ready |
...
bindings still need their consumers. The task queue and its pointer are unchanged.
...
| Default document/license viewer | `license_viewer_snapshot` qualified the GNOME Text Editor handler reached from Parent About in case 151, run `20260922T220454Z-80a92d46`: public `view` ID, bounded GPL headings, active viewer, close/absence, owned About/footer and unchanged Parent settings. Ambiguity, wrong document/owner, masked/hidden content, incomplete trees and uncertain input have host regressions. Qualification is limited to the pinned Ubuntu 26.04 environment; no separate editor-version/locale matrix or live negative-entry matrix was run. Other handlers and document workflows remain pending. | Parent ABOUT02/03 binding ready; task 185l's remaining live refusal qualification, FILE08 and FEED08 pending |
| Terminal | The standard-account Ptyxis binding passed the former terminal-based case 6 route. That historical run covered desktop wrong-surface refusal, Ctrl+Alt+T entry, focused input, closure, independent second entry and GUI denial. `standard_terminal_snapshot` retains its public application ID and scoped registry ownership checks, rejecting ambiguous, foreign and incomplete surfaces. Current case 6 consumes PARENT01 directly and supplies no terminal qualification. Generic text, administrator input, other providers and multiwindow/tab routes remain unqualified. INFO02 uses guarded SSH stdout. | Historical standard Ptyxis entry/close scope only; current FILE01/02/06 consumers and LIFE04 pending |
...
The historical terminal report records product, infrastructure, collection and cleanup
