# Retained engineering reuse and qualification

**Scope — 2026-09-14:** this preserves the former reuse map and qualification
limits for engineering and mechanical installation work. It is not the active
customer routing map. Old scheduling, probe next actions and backend acceptance
instructions below do not govern customer E2E. Use [Reuse-Map.md](Reuse-Map.md)
for current task routing and [Policy-Acknowledgement.md](Policy-Acknowledgement.md)
for the separate deferred product design. Mechanical Tasks 18/20 may reuse the
installation records without weakening their existing guards or checkpoints.
...
- Native/Flatpak process and package assets:
  ...
  Builder/controller source enumeration now shares the exact operator-output
  exclusion; the contract links synthetic no-open/no-copy, digest-equivalence
  and other-source mutation regressions. Fresh build and the installed lifecycle
  consumer now pass with retained evidence in the contract; graphical-controller
  qualification remains separate. Other documentation remains an input and R1
  ownership/timing is unchanged.
...
- Execution-policy notification and rollback recovery: reuse
  ...
  its safety/native regressions. The owning admission contract now also records
  `ExecutionProbe._admission_binding` and its `test_admission_*` synthetic-manager
  regressions: retained-reply validation, captured-manager/PID/unit/invocation
  matching and metadata/replacement refusal. It does not send admission or
  qualify real systemd. The same contract now owns `ProbeGeneration`: exclusive
  private copies, pinned path/inode/digest verification and replacement-safe
  cleanup retries, covered by `test_probe_generation_cleanup_safety.py` and
  `test_generation_owner_retains_native_witness_through_channel_settlement`.
  The latter qualifies local native exec with synthetic manager coordinates.
  `ExecutionProbe.run_native/recover` now integrates generation/binding retention,
  one-shot admission, shared dispatch deadlines and terminal/cleanup comparison.
  Its `test_native_*` lifecycle cases use synthetic manager/channel services;
  native command serialization/late replies have separate private-bus coverage.
  The owning contract distinguishes these from the now-passing guarded native
  lifecycle composition. Payload compilation/install, activation digests and a
  preserved private runtime-directory declaration now have staged-package and
  relocated-removal regressions in the owning contract. The first guarded attempt
  proves installed directory provisioning but fails at uncertain creation before
  admission; outer baseline cleanup passed. The second attempt identifies an API
  `PropertyReadOnly` rejection, again with incomplete adapter recovery and verified
  outer cleanup. The owning contract records the inferred `JobTimeoutUSec` setter
  defect and the package drop-in correction, with effective-value admission
  refusal and terminal cleanup regressions. Guarded attempt 3 now qualifies real
  success/withheld-admission/fresh-success, effective 4,000,000 µs timeout in each
  stage, and owned cleanup; the contract retains its result/JUnit and both earlier
  failures. The owning contract also records passing broker mount/storage
  retention, post-terminal sender-loss settlement and
  `system_probe_sandbox.native_probe_broker_sandbox` qualification under the
  packaged service's complete execution restrictions. Reuse its guarded
  `ExecStartPost` worker and `test_system_probe_sandbox_cleanup_safety.py` for
  input relocation, effective restrictions and exact drop-in restoration;
  the contract links all three installed results and their distinct limits.
  Reuse the [final-byte activation manifest hook](../Publishing.md#package-update-activation)
  and `test_final_package_hook_refreshes_stripped_binary_digests`: install-time
  hashes became stale after debhelper stripped ELF payloads. The corrected Debian
  archive passes every activation digest; failed/corrected build identities are
  retained in the active 15A handoff. This is not test-artifact provenance reuse.
  Unknown creation identities are never adopted.
  Pre-terminal permanent-loss settlement, broker restart ownership,
  rule-attributed causal binding and installed policy qualification remain open;
  this qualification path is not a generation receipt. Its
...
- Local publishing validation: `make test-all` and `make test-all-verify`
  share [one module](../Publishing.md#local-publishing-tests)
...
- Aggregate execution: reuse the [bounded host scheduler contract](../../tests/README.md#all-established-regressions)
  and [scheduling policy](Test-All-Parallelism-Design.md#schedule-and-selection).
  ...
  Preserve the [progress event framing contract](Test-All-Parallelism-Design.md#progress-event-framing):
  `regression_events.emit` writes each delimited record together, and test-owned
  background dispatch must finish before teardown.
  ...
  Publishing and artifact operations use the scheduler's explicit companion list;
...
| [15A](Task-15.md#task-15a) | Extend F1's installed enforcement area and native/Snap/Flatpak case data; reuse the [notification recovery contract](../SystemDesign/Applications.md#notification-recovery-and-acknowledgement-limit), [generation-witness gate](../SystemDesign/Applications.md#generation-witness-design-gate), [rule-decision correlation and local regressions](../SystemDesign/Applications.md#rule-decision-correlation-boundary), [bounded transport/recovery and dispatch table](../SystemDesign/Applications.md#bounded-probe-transport-and-open-limits), [owned-client integration](Evidence/15A-Probe-Client-Integration-20260911.md), [evidence-retention correction](Evidence/15A-Probe-Evidence-Retention-20260911.md), [retained replies](Evidence/15A-Probe-Retained-Reply-20260911.md), [job-identity refusal](Evidence/15A-Probe-Job-Identity-20260911.md), [admission, generation ownership, broker storage and service sandbox qualification](../SystemDesign/Applications.md#pre-exec-admission-for-a-causal-witness), [permanent sender-loss settlement](../SystemDesign/Applications.md#settlement-after-permanent-sender-loss), [native protocol qualification](Evidence/15A-Probe-Native-Admission-20260911.md), [broker channel qualification](Evidence/15A-Probe-Broker-Channel-20260911.md), [final-byte package activation](../Publishing.md#package-update-activation) and [dependency capture/audit](Evidence/15A-Activation-Interface-Audit-20260911.md); publish launch witnesses for 25A. | Every required platform, route, identity/matching boundary and other-user outcome; notification, early markers and nonce denials cannot qualify activation. Local/private-bus regressions qualify the adapter, native protocol, generation owner and mismatch/replacement refusals at their stated layers. Composition attempt 3 qualifies native success/withheld-admission/fresh-success, timeout and cleanup; storage attempt 1 proves broker mount access and exact fixture-owned generation preservation across stop/start and restart. Sender-loss attempt 1 qualifies collection after copied terminal evidence and a successful create reply, then fresh success; earlier loss remains locally qualified uncertainty, not finite settlement. Sandbox attempt 1 qualifies all three native stages in the packaged service's start-post execution context, effective restrictions and restored configuration; it does not qualify broker event-loop integration. Both earlier uncertain-creation failures remain retained; attempt 2's PropertyReadOnly property identity is inferred. The packaged drop-in and [source-selection correction](../../tests/integration/README.md#package-and-fixture-inputs) have fresh-build/installed evidence. Positive correlation now has synthetic local coverage; no live journal collector, deny control or activation receipt is qualified. Endpoint equality cannot exclude intervening input restoration, and the narrow fixture grammar does not certify arbitrary administrator rules. Broker ownership recovery, pre-terminal loss settlement, policy attribution and the full native/Snap/Flatpak matrix remain open. |
...
| [18A](Task-18.md#task-18a) | Build each required activation fixture once per verified build-input set; reuse the [final-byte activation manifest hook and regression](../Publishing.md#package-update-activation), parameterize assertions and extend applicable [installation observations](#installation-helper-and-open-limits). | All four activation classes, changed/added/removed files, real process/session/reboot transitions. |
...
- **Scenario ownership:** E2E-005's six pending transaction variants belong to
  21B; 21A retains independently verifiable discovery/validation. E2E-006 shares
  family ownership with 25A. E2E-026 update variants belong to 18A and E2E-027's
  continuous removal variant to 18C. The inventory and pending reasons match
  the customer coverage table; IDs, actions, assertions and readiness remain
  unchanged. `test_e2e_inventory.py` checks family ownership and validates each
  variant's owner. Link shared execution results; partial screens or ownership
  reconciliation do not earn a complete scenario pass. Legacy fault-family
  transfers still require reconciliation by their first affected consumers.
