...
| [ ] | G03 / 40 min | G01, P02 | **VM diagnosis:** inspect only the offered station entry branch in the guarded envelope. Record whether session choice is offered and its public controls, or the destination of the default branch. Do not activate an unresolved choice. Ordinary-account selection reaches its prompt and Escape returns to the list. Collect evidence/cleanup; this observation task does not mark REQUEST01 ready. **Blocker:** [run 20260921T064650Z-ff9d8201](../Evidence/test-all-runs/20260921T064650Z-ff9d8201/report.md) reconnected the detached console and executed the observer through stdin, then refused the first greeter read with `ui:gdm-account-cardinality`; ordinary prompt/Escape/list and station branch were not reached. Failure artifacts were retained; collection was `not-run`, cleanup passed, lease completed and VM was confirmed off. **Return:** establish why the owned greeter tree does not resolve exactly one ordinary and one station row, preserve ambiguity refusal, then rerun the same prepared qualification through those observations and cleanup. No rebuild or snapshot replacement. |
