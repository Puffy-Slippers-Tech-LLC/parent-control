# 016b — Start and collect a trace of an unchanged public state

Estimate: 40–60 minutes. Aim for one session; this is not a stop timer.
Follow the [session contract](../E2E-Execution-Plan.md#task-size-and-order).

Session exception: The shared observer lifecycle changes recorder ordering; live token qualification and affected retained regressions cannot be replaced by host checks.

## Scope and prerequisites

Deliver **UI25/26 observer readiness, token lifetime and stable-state collection**. Named consumer: task **016** and any
...
Implement trace readiness and collection together in the existing rendezvous. Bind a token, bounded public projection, monotonic deadline and terminal predicate without any intervening input.
...
```sh
tools/run-tests integration check_e2e_trace_stable_state
```
...
acceptance and cleanup pass, check **016b**, advance the sole pointer to the
