...
Next task: customer **012 — [Select kiosk accounts and read availability](../E2E-Tasks/012-request-choices.md)**,
the first eligible capability after task 017's Parent saved/control qualification.
...
Run task 012 under its customer brief to qualify kiosk child/approver choices
and unavailable state. After the released customer work is complete,
...
and their GDM returns. Tasks 010 and 017 qualified the Parent Screen time limit
toggle and saved/control snapshots; customer task 012 is now the first eligible work.
