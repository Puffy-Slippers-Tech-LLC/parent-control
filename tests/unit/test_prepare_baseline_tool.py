...
def test_failed_baseline_does_not_refresh_test_tools(authorized, monkeypatch):
    run = Mock(return_value=SimpleNamespace(returncode=1))
    monkeypatch.setattr(authorized['subprocess'], 'run', run)
    assert authorized['main']([]) == 1
    run.assert_called_once()
    assert run.call_args.args[0] == [
        '/usr/bin/pkexec', authorized['HELPER'], 'prepare-baseline']
    assert run.call_args.kwargs['env']['PATH'] == '/usr/sbin:/usr/bin:/sbin:/bin'


def test_successful_baseline_refreshes_test_tools(authorized, monkeypatch):
    run = Mock(side_effect=[SimpleNamespace(returncode=0), SimpleNamespace(returncode=0)])
    monkeypatch.setattr(authorized['subprocess'], 'run', run)
    assert authorized['main']([]) == 0
