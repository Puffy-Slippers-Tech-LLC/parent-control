...
| GDM02 | C | Select a named user on the logon screen. Use UI14's qualified provider navigation, verify focus before Enter, then observe the declared password prompt, retained lock or passwordless station. Do not type a password. | `check_e2e_kiosk_entry` installed-qualifies only `gdm_nonsecret_navigation(station)` followed by one Enter and independent `station_default_entry` owned-form readback. Other accounts, password prompts, retained locks and session choices remain unqualified; the retained generic positional Home/Down calculation stays retired. | pending; passwordless station binding ready |
