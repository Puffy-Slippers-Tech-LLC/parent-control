...
| REQUEST04 | C | Select a named form field: child, approver or duration choice. Observe loaded selection and control availability. Reject child selection on the fixed-child overlay. | `AccessibleUI.select_kiosk_account(field, name, expected=...)` opens the owned selector, checks the exact offered UID set and labels, invokes the chosen public action once, then waits for fresh REQUEST03 selected-value readback without replay. `kiosk_eligible_choices.PLAN` / `onpc_kiosk_eligible_choices::run` qualified selecting the enabled child and intended approver after public Parent enable/save and Switch User; wrong-entry and wrong/absent choice refusals, private collection and owned cleanup passed through `check_e2e_kiosk_eligible_choices` in run `20260923T204614Z-0601b77a`. Selecting a disabled child, other account profiles, duration editing and installed overlay refusal remain pending. | pending; enabled kiosk child/approver selection ready |
