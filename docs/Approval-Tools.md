...
| Scripted E2E implementation | `tools/write-e2e [--sessions N]` / `tools/write-e2e --stop` | Fresh Astra Low implementation and Astra High live/repair sessions; omitted limit is unlimited; attach to a live owner before interpreting new arguments; safe stop at the next session boundary, Ctrl+C owned cancellation; task-scoped staging without commits; existing grants and guarded test cleanup |
